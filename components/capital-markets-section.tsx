'use client'

import Link from 'next/link'
import { TrendingUp, PiggyBank, FileCheck } from 'lucide-react'
import { motion, useInView } from 'framer-motion'
import { useRef } from 'react'

const WA_LINK = 'https://wa.link/69asix'

const points = [
  {
    icon: <TrendingUp size={18} strokeWidth={1.8} />,
    title: 'Financiamiento a tu medida',
    desc: 'Diseñamos la estrategia de endeudamiento óptima y elegimos la mejor fuente.',
  },
  {
    icon: <PiggyBank size={18} strokeWidth={1.8} />,
    title: 'Inversión de excedentes',
    desc: 'Hacé rendir la liquidez de tu empresa con análisis de rentabilidad y gestión del riesgo.',
  },
  {
    icon: <FileCheck size={18} strokeWidth={1.8} />,
    title: 'Legajos y garantías',
    desc: 'Armamos legajos crediticios profesionales y gestionamos avales para aprovechar más rápido y en mejores condiciones.',
  },
]

export default function CapitalMarketsSection() {
  const ref = useRef(null)
  const inView = useInView(ref, { once: true, margin: '-80px' })

  return (
    <section className="py-24 px-6 bg-wl-navy relative overflow-hidden">
      {/* Subtle background elements */}
      <div className="pointer-events-none absolute inset-0 overflow-hidden" aria-hidden="true">
        <div className="absolute -top-32 -right-32 w-[400px] h-[400px] rounded-full bg-wl-cyan/8 blur-3xl" />
        <div className="absolute -bottom-20 -left-20 w-[300px] h-[300px] rounded-full bg-wl-cyan/5 blur-3xl" />
        {/* Grid pattern overlay */}
        <div
          className="absolute inset-0 opacity-[0.04]"
          style={{
            backgroundImage: 'radial-gradient(circle, #1595BC 1px, transparent 1px)',
            backgroundSize: '32px 32px',
          }}
        />
      </div>

      <div ref={ref} className="relative max-w-5xl mx-auto">
        <div className="grid lg:grid-cols-2 gap-14 items-center">
          {/* Left: Text */}
          <div>
            <motion.span
              initial={{ opacity: 0, y: 12 }}
              animate={inView ? { opacity: 1, y: 0 } : {}}
              transition={{ duration: 0.45 }}
              className="inline-flex items-center text-wl-cyan font-body font-[600] text-xs tracking-widest uppercase bg-wl-cyan/15 border border-wl-cyan/25 rounded-full px-4 py-1.5"
            >
              Lo que nos hace diferentes
            </motion.span>

            <motion.h2
              initial={{ opacity: 0, y: 20 }}
              animate={inView ? { opacity: 1, y: 0 } : {}}
              transition={{ duration: 0.55, delay: 0.1 }}
              className="mt-5 font-sans font-[700] text-3xl sm:text-4xl text-white leading-tight text-balance"
            >
              Conectamos tu PyME al mercado de capitales
            </motion.h2>

            <motion.p
              initial={{ opacity: 0, y: 20 }}
              animate={inView ? { opacity: 1, y: 0 } : {}}
              transition={{ duration: 0.55, delay: 0.18 }}
              className="mt-4 font-body text-[15px] text-white/65 leading-relaxed"
            >
              Te abrimos las puertas a fuentes de financiamiento e inversión que suelen quedar reservadas al mundo corporativo, para que diversifiques, reduzcas tu costo financiero y hagas rendir tu tesorería.
            </motion.p>

            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={inView ? { opacity: 1, y: 0 } : {}}
              transition={{ duration: 0.55, delay: 0.28 }}
              className="mt-8"
            >
              <Link
                href={WA_LINK}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 bg-wl-green hover:bg-[#178035] text-white font-body font-[600] text-sm px-6 py-3 rounded-xl transition-all duration-200 shadow-md hover:shadow-xl hover:-translate-y-0.5"
              >
                <svg width="15" height="15" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z" />
                  <path d="M12 0C5.373 0 0 5.373 0 12c0 2.123.554 4.116 1.523 5.847L.057 23.882l6.197-1.425A11.945 11.945 0 0012 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 21.818a9.785 9.785 0 01-5.044-1.4l-.361-.215-3.741.981.998-3.648-.235-.374A9.817 9.817 0 012.182 12C2.182 6.579 6.579 2.182 12 2.182S21.818 6.579 21.818 12 17.421 21.818 12 21.818z" />
                </svg>
                Descubrí las oportunidades para tu PyME
              </Link>
            </motion.div>
          </div>

          {/* Right: Points */}
          <div className="flex flex-col gap-4">
            {points.map((p, i) => (
              <motion.div
                key={p.title}
                initial={{ opacity: 0, x: 24 }}
                animate={inView ? { opacity: 1, x: 0 } : {}}
                transition={{ duration: 0.55, delay: 0.15 + i * 0.12 }}
                className="flex gap-4 bg-white/6 border border-white/10 rounded-2xl p-5 hover:bg-white/9 transition-all duration-200"
              >
                <div className="w-9 h-9 rounded-lg bg-wl-cyan/20 flex items-center justify-center text-wl-cyan shrink-0 mt-0.5">
                  {p.icon}
                </div>
                <div>
                  <h3 className="font-sans font-[700] text-white text-[14px] mb-1">
                    {p.title}
                  </h3>
                  <p className="font-body text-sm text-white/60 leading-relaxed">
                    {p.desc}
                  </p>
                </div>
              </motion.div>
            ))}
          </div>
        </div>
      </div>
    </section>
  )
}
